-- Cargo del autor junto al nombre: Trabajador / Administrador / Desarrollador
-- Fecha: 25/08/2026
-- Si la columna ya existe (instalación nueva con 5.mensajes_academico.sql), ignora el error 1060.

USE `AcademiaDB`;

ALTER TABLE MENSAJE
    ADD COLUMN CARGO VARCHAR(30) NOT NULL DEFAULT 'Administrador' AFTER DESTINATARIO;

UPDATE MENSAJE
SET CARGO = 'Administrador'
WHERE TRIM(IFNULL(CARGO, '')) = '';
...
main: BEGIN
    ...
    SELECT
        m.IDMENSAJE, m.TITULO, m.MENSAJE, m.FECHAINICIO, m.FECHAFIN,
        m.DESTINATARIO, m.CARGO, m.ESTADO, m.CREADO_POR,
        TRIM(CONCAT(IFNULL(u.NOMBRE, ''), ' ', IFNULL(u.APELLIDO, ''))) AS AUTOR,
        m.FECHACREACION, m.HORACREACION, m.MODIFICADO_POR, m.FECHAMODIFICACION, m.HORAMODIFICACION
    FROM MENSAJE m
    ...
    ORDER BY
        ...
        CASE WHEN p_OrdenarPor = 'CARGO' AND p_Direccion = 'ASC' THEN m.CARGO END ASC,
        CASE WHEN p_OrdenarPor = 'CARGO' AND p_Direccion = 'DESC' THEN m.CARGO END DESC,
...
main: BEGIN
    ...
    SET v_Hora = TIME_FORMAT(IFNULL(CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '-05:00'), DATE_SUB(UTC_TIMESTAMP(), INTERVAL 5 HOUR)), '%H:%i:%s');
    ...
    IF p_Titulo IS NULL OR TRIM(p_Titulo) = '' THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa el título.'; LEAVE main; END IF;
    IF p_Mensaje IS NULL OR TRIM(p_Mensaje) = '' THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa el mensaje.'; LEAVE main; END IF;
    IF p_FechaInicio IS NULL OR CHAR_LENGTH(p_FechaInicio) <> 8 THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa la fecha de inicio.'; LEAVE main; END IF;
    IF p_FechaFin IS NULL OR CHAR_LENGTH(p_FechaFin) <> 8 THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa la fecha final.'; LEAVE main; END IF;
    IF STR_TO_DATE(p_FechaFin, '%d%m%Y') < STR_TO_DATE(p_FechaInicio, '%d%m%Y') THEN SET p_Resultado = 0; SET p_MensajeOut = 'La fecha final no puede ser anterior al inicio.'; LEAVE main; END IF;
    IF v_Dest IS NULL OR v_Dest NOT IN ('Estudiantes', 'Trabajadores', 'Todos') THEN SET p_Resultado = 0; SET p_MensajeOut = 'Selecciona a quién va el mensaje.'; LEAVE main; END IF;
    IF v_Cargo NOT IN ('Trabajador', 'Administrador', 'Desarrollador') THEN SET p_Resultado = 0; SET p_MensajeOut = 'Selecciona el cargo del autor.'; LEAVE main; END IF;
    SELECT IFNULL(MAX(CAST(REPLACE(IDMENSAJE, 'MSG', '') AS UNSIGNED)), 0) + 1 INTO v_Next FROM MENSAJE WHERE IDMENSAJE LIKE 'MSG%';
    SET p_IdGenerado = CONCAT('MSG', LPAD(CAST(v_Next AS CHAR), 3, '0'));
    INSERT INTO MENSAJE (IDMENSAJE, TITULO, MENSAJE, FECHAINICIO, FECHAFIN, DESTINATARIO, CARGO, ESTADO, CREADO_POR, FECHACREACION, HORACREACION)
    VALUES (p_IdGenerado, TRIM(p_Titulo), TRIM(p_Mensaje), p_FechaInicio, p_FechaFin, v_Dest, v_Cargo, v_Estado, @audit_id_usuario, v_Fecha, v_Hora);
...
main: BEGIN
    ...
    SET v_Cargo = IFNULL(NULLIF(TRIM(p_Cargo), ''), 'Administrador');
    SET v_Estado = IFNULL(NULLIF(TRIM(p_Estado), ''), 'Activo');
    IF NOT EXISTS (SELECT 1 FROM MENSAJE WHERE IDMENSAJE = p_Id) THEN SET p_Resultado = 0; SET p_MensajeOut = 'El mensaje no existe.'; LEAVE main; END IF;
    IF p_Titulo IS NULL OR TRIM(p_Titulo) = '' THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa el título.'; LEAVE main; END IF;
    IF p_Mensaje IS NULL OR TRIM(p_Mensaje) = '' THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa el mensaje.'; LEAVE main; END IF;
    IF p_FechaInicio IS NULL OR CHAR_LENGTH(p_FechaInicio) <> 8 THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa la fecha de inicio.'; LEAVE main; END IF;
    IF p_FechaFin IS NULL OR CHAR_LENGTH(p_FechaFin) <> 8 THEN SET p_Resultado = 0; SET p_MensajeOut = 'Ingresa la fecha final.'; LEAVE main; END IF;
    ...
    IF v_Cargo NOT IN ('Trabajador', 'Administrador', 'Desarrollador') THEN SET p_Resultado = 0; SET p_MensajeOut = 'Selecciona el cargo del autor.'; LEAVE main; END IF;
    UPDATE MENSAJE SET
        TITULO = TRIM(p_Titulo), MENSAJE = TRIM(p_Mensaje),
        FECHAINICIO = p_FechaInicio, FECHAFIN = p_FechaFin,
        DESTINATARIO = v_Dest, CARGO = v_Cargo, ESTADO = v_Estado,
        MODIFICADO_POR = @audit_id_usuario, FECHAMODIFICACION = v_Fecha, HORAMODIFICACION = v_Hora
...
main: BEGIN
    ...
    SELECT
        m.IDMENSAJE, m.TITULO, m.MENSAJE, m.DESTINATARIO,
        IFNULL(NULLIF(TRIM(m.CARGO), ''), 'Administrador') AS CARGO,
        m.FECHAINICIO, m.FECHAFIN,
        TRIM(CONCAT(IFNULL(u.NOMBRE, ''), ' ', IFNULL(u.APELLIDO, ''))) AS AUTOR,
        m.FECHACREACION, m.HORACREACION
    FROM MENSAJE m
    LEFT JOIN USUARIO u ON u.IDUSUARIO = m.CREADO_POR
    WHERE m.ESTADO = 'Activo'
      AND CHAR_LENGTH(m.FECHAINICIO) = 8 AND CHAR_LENGTH(m.FECHAFIN) = 8
...
SELECT 'MENSAJE.CARGO (Trabajador / Administrador / Desarrollador) listo.' AS info;
