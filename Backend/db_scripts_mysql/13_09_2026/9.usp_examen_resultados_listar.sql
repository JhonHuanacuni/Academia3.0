...
main: BEGIN
    ...
    WHERE (v_EsEstudiante = 0 OR i.IDUSUARIO = p_IdSolicitante)
      ...
      AND (
          ...
          IFNULL(u.DNI, '') LIKE CONCAT('%', p_Buscar, '%') OR
          IFNULL(u.NOMBRE, '') LIKE CONCAT('%', p_Buscar, '%') OR
          IFNULL(u.APELLIDO, '') LIKE CONCAT('%', p_Buscar, '%') OR
