-- Convertido desde db_scripts/26_07_2026/8.asesor_registro_mensualidad.sql
...
SET @sql_ASESOR_IDUSUARIO := IF(@col_ASESOR_IDUSUARIO = 0,
    'ALTER TABLE ASESOR ADD IDUSUARIO VARCHAR(50) NULL',
    'SELECT 1');
...
SET @sql_FK_ASESOR_USUARIO := IF(@fk_FK_ASESOR_USUARIO = 0,
    'ALTER TABLE ASESOR ADD CONSTRAINT FK_ASESOR_USUARIO FOREIGN KEY (IDUSUARIO) REFERENCES USUARIO(IDUSUARIO)',
    'SELECT 1');
...
main: BEGIN
    ...
    IF p_IdUsuario IS NOT NULL AND p_IdUsuario <> ''
       AND NOT EXISTS (SELECT 1 FROM USUARIO WHERE IDUSUARIO = p_IdUsuario) THEN
        SET p_Resultado = 0; SET p_Mensaje = 'El usuario vinculado no existe.';
        LEAVE main;
    END IF;
    ...
    IF p_IdUsuario IS NOT NULL AND p_IdUsuario <> ''
       AND EXISTS (SELECT 1 FROM ASESOR WHERE IDUSUARIO = p_IdUsuario AND ACTIVO = 1) THEN
        SET p_Resultado = 0; SET p_Mensaje = 'Ese usuario ya está vinculado a otro asesor activo.';
        LEAVE main;
    END IF;
...
main: BEGIN
    ...
    IF p_IdUsuario IS NOT NULL AND p_IdUsuario <> ''
       AND EXISTS (SELECT 1 FROM ASESOR WHERE IDUSUARIO = p_IdUsuario AND IDASESOR <> p_Id AND ACTIVO = 1) THEN
        SET p_Resultado = 0; SET p_Mensaje = 'Ese usuario ya está vinculado a otro asesor activo.';
        LEAVE main;
    END IF;
...
main: BEGIN
    ...
    SELECT
        ...
        UPPER(TRIM(
            COALESCE(
                (SELECT a.NOMBRE FROM ASESOR a WHERE a.IDUSUARIO = m.REGISTRADOPOR AND a.ACTIVO = 1 LIMIT 1),
