-- ============================================================================
-- Pagos: listar, abonar membresía, membresías del estudiante — MySQL 8
-- ============================================================================
...
CREATE PROCEDURE usp_pago_membresias_estudiante(IN p_IdUsuario VARCHAR(50))
main: BEGIN
    SELECT
        m.IDMEMBRESIA,
        m.IDPLAN,
...
main: BEGIN
    ...
    IF p_IdMembresia IS NULL OR p_IdMembresia = '' THEN
        SET p_Resultado = 0; SET p_Mensaje = 'Debe seleccionar una membresía.'; LEAVE main;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM MEMBRESIA WHERE IDMEMBRESIA = p_IdMembresia AND ESTADO = 'Activo') THEN
        SET p_Resultado = 0; SET p_Mensaje = 'La membresía no existe o está inactiva.'; LEAVE main;
    END IF;
    ...
    FROM MEMBRESIA WHERE IDMEMBRESIA = p_IdMembresia;

    SELECT IFNULL(SUM(MONTO), 0) INTO v_Pagado
    FROM PAGOMEMBRESIA WHERE IDMEMBRESIA = p_IdMembresia;

    SET v_Deuda = v_MontoTotal - v_Pagado;
    IF v_Deuda < 0 THEN SET v_Deuda = 0; END IF;

    IF v_Deuda <= 0 THEN
        SET p_Resultado = 0; SET p_Mensaje = 'Esta membresía no tiene deuda pendiente.'; LEAVE main;
    END IF;
...
SELECT 'SPs de pagos creados.' AS info;
