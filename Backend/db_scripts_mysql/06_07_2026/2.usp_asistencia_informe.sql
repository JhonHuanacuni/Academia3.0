-- Convertido automáticamente desde db_scripts/06_07_2026/2.usp_asistencia_informe.sql
-- MySQL 8 — Academia 3.0

USE `AcademiaDB`;

/* ============================================================================
   Informe de asistencias por rango de fechas
   Ejecutar después de 1.modulo_informes.sql
   Fecha: 06/07/2026
   ============================================================================ */
...
IF p_FechaDesde IS NULL OR p_FechaDesde = '' OR p_FechaHasta IS NULL OR p_FechaHasta = '' THEN
    ...
    /* Estudiantes activos con membresía vigente en el rango (o sin filtro de membresía) */
    SELECT
        u.IDUSUARIO,
        UPPER(TRIM(
    ...
    /* Marcas de asistencia en el rango */
    SELECT
        a.IDUSUARIO,
        a.FECHAREGISTRO,
        a.ESTADO,
...
SELECT 'usp_asistencia_informe creado correctamente.';
END$$
