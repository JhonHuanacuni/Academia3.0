...
def listar_submodulos_modulo_usuario(idusuario: str, idmodulo: str):
    """Submódulos de un módulo con flag asignado (visible para el usuario)."""
...
def get_menu_for_user(idusuario: str):
    """Menú del sidebar según módulos efectivos del usuario (rol + asignaciones - exclusiones)."""
...
def asignar_modulo_usuario_orm(idusuario: str, idmodulo: str):
    """Fallback sin SP si aún no se ejecutó modulos_admin.sql."""
    UsuarioModuloExcluido.objects.filter(
