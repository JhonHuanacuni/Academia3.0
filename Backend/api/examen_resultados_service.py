...
def detalle_resultado(id_intento: str, id_solicitante: str):
    ...
    return {
        'intento': {
            'IDINTENTOEXAMEN': intento.get('IDINTENTOEXAMEN'),
            'IDEXAMEN': intento.get('IDEXAMEN'),
            'EXAMEN': intento.get('EXAMEN') or '',
            'IDUSUARIO': intento.get('IDUSUARIO'),
            'ESTUDIANTE': intento.get('ESTUDIANTE') or '',
            'DNI': intento.get('DNI') or '',
            'NUMEROINTENTO': intento.get('NUMEROINTENTO'),
            'FECHAINICIO': intento.get('FECHAINICIO') or '',
            'HORAINICIO': intento.get('HORAINICIO') or '',
            'FECHAFIN': intento.get('FECHAFIN') or '',
            'HORAFIN': intento.get('HORAFIN') or '',
            'PUNTAJEOBTENIDO': _serialize_decimal(intento.get('PUNTAJEOBTENIDO')),
            'PUNTAJETOTAL': _serialize_decimal(intento.get('PUNTAJETOTAL')),
            'PUNTAJEAPROBADO': _serialize_decimal(intento.get('PUNTAJEAPROBADO')),
            'CANTCORRECTAS': intento.get('CANTCORRECTAS'),
            'CANTINCORRECTAS': intento.get('CANTINCORRECTAS'),
            'CANTSINRESPONDER': intento.get('CANTSINRESPONDER'),
            'APROBADO': bool(intento.get('APROBADO')) if intento.get('APROBADO') is not None else None,
        },
        'preguntas': preguntas,
        'soloPropios': solo_propios,
    }
...
def catalogos_resultados(id_solicitante: str):
    """Exámenes y aulas para filtros del listado."""
    solo_propios = _es_estudiante((id_solicitante or '').strip())
    with connection.cursor() as cursor:
        if solo_propios:
            cursor.execute(
                """
    ...
    return {
        'examenes': examenes,
        'aulas': [] if solo_propios else aulas,
        'soloPropios': solo_propios,
    }
