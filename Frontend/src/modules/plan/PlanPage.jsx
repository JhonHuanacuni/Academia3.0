...
export default function PlanPage() {
  ...
  const abrirEliminar = (row) => {
    setConfirm({
      id: row[cfg.pk],
      mensaje: `¿Eliminar el plan «${row.NOMBRE || row[cfg.pk]}»? Esta acción no se puede deshacer.`,
    });
  };
  ...
  const handleGuardar = async (payload) => {
    const body = {
      ...payload,
      DIASASISTENCIA: Number(payload.DIASASISTENCIA) || 63,
    };
    let mensaje;
    if (modo === "crear") {
  ...
  const tituloModal =
    modo === "crear" ? "Nuevo plan" : modo === "editar" ? "Editar plan" : "Ver plan";
