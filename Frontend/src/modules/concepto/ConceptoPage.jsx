...
import { conceptoConfig } from "./concepto.config";
...
export default function ConceptoPage() {
  ...
  const abrirEliminar = (row) => {
    setConfirm({
      id: row[cfg.pk],
      mensaje: `¿Eliminar el concepto «${row.NOMBRE || row[cfg.pk]}»? Esta acción no se puede deshacer.`,
    });
  };
  ...
  const handleGuardar = async (payload) => {
    const body = {
      ...payload,
      COSTO: payload.COSTO === "" || payload.COSTO == null ? 0 : Number(payload.COSTO),
    };
    let mensaje;
    if (modo === "crear") {
  ...
  const tituloModal =
    modo === "crear" ? "Nuevo concepto" : modo === "editar" ? "Editar concepto" : "Ver concepto";
