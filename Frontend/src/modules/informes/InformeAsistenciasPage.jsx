...
import { inputToDb, primerDiaMesInput, ultimoDiaMesInput, rangoMesCompletoInput } from "../../utils/fecha";
...
export default function InformeAsistenciasPage() {
  ...
  const exportarExcel = useCallback(async () => {
    ...
    const mes = rangoMesCompletoInput(fechaDesde);

    const desdeDb = inputToDb(mes.desde);

    const hastaDb = inputToDb(mes.hasta);
  ...
  }, [fechaDesde, buscar, idPlan, estado]);
  ...
  const rangoLabel = useMemo(() => {
    ...
    return `${total} estudiante${total !== 1 ? "s" : ""} en el período`;
        ...
        <div className="informes-panel">
          ...
          <div className="informes-panel-body" role="tabpanel">
            {tabActiva === "indicadores" ? (
              ...
              <InformeAsistenciasTabla filas={filas} dias={dias} />
            )}
          </div>
        </div>
