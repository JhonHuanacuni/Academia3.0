...
import { bibliotecaConfig } from "./biblioteca.config";
...
import BibliotecaVerPage from "./BibliotecaVerPage";
import "../../styles/mantenedor.css";
import "./biblioteca.css";

export default function BibliotecaPage({ role }) {
  const cfg = bibliotecaConfig;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/libros/catalogos/");
  ...
  const abrirEditar = async (row) => {
    try {
      const data = await obtenerDocumento(row[cfg.pk]);
  ...
  const abrirVer = async (row) => {
    try {
      ...
      const data = await obtenerDocumento(row[cfg.pk]);
      if (!data?.URLPDF) {
        setToast({ mensaje: "Este documento no tiene PDF asociado.", tipo: "error" });
        return;
      }
      setDocumentoVer({ url: data.URLPDF, titulo: data.TITULO });
      setVista("ver");
    } catch (err) {
      setToast({ mensaje: err.message, tipo: "error" });
    } finally {
  ...
  const abrirEliminar = (row) => {
    setConfirm({
      id: row[cfg.pk],
      mensaje: `¿Eliminar «${row.TITULO || row[cfg.pk]}»? Se borrará el archivo del servidor.`,
    });
  };

  const enviarForm = async ({ TITULO, DESCRIPCION, ESTADO, AULAS, archivo, portada }) => {
  ...
  const tituloModal =
    modo === "crear" ? "Agregar PDF" : modo === "editar" ? "Editar PDF" : "Ver PDF";

  if (vista === "ver" && documentoVer) {
    return (
      <>
        <BibliotecaVerPage
          titulo={documentoVer.titulo}
          url={documentoVer.url}
