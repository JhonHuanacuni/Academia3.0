...
function AuditoriaDetalleModal({ abierto, registro, loading, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        className="modal-panel auditoria-detalle-modal"
            ...
            <>
              <dl className="auditoria-meta">
                <div className="auditoria-meta-item">
                  <dt>Fecha / Hora</dt>
                  <dd>
                    {dbToView(String(registro.FECHA || "")) || "—"} {registro.HORA || ""}
                  </dd>
                </div>
                <div className="auditoria-meta-item">
                  <dt>Tabla</dt>
                ...
                <div className="auditoria-meta-item">
                  <dt>Usuario</dt>
                  <dd>
                    {[registro.USUARIO_NOMBRE, registro.IDUSUARIO].filter(Boolean).join(" · ") || "—"}
                  </dd>
                </div>
                <div className="auditoria-meta-item auditoria-meta-item--full">
                  <dt>ID registro</dt>
                  <dd className="auditoria-meta-id" title={registro.IDREGISTRO || ""}>
                    {registro.IDREGISTRO || "—"}
                  </dd>
                </div>
              </dl>
...
export default function AuditoriaPage() {
  ...
  return (
    <div className="mantenedor-page">
      <PageHeader titulo={cfg.titulo} descripcion={cfg.descripcion} />

      <div className="mantenedor-card">
        <Toolbar
          buscar={crud.buscar}
          onBuscarChange={crud.onBuscarChange}
          placeholder="Buscar tabla, registro, usuario..."
        />
        ...
        <div className="auditoria-filtros">
          <label>
            Tabla
            <select
              value={crud.filtros.tabla || ""}
              onChange={(e) => crud.setFiltro("tabla", e.target.value)}
            >
          ...
          <label>
            Acción
            <select
              value={crud.filtros.accion || ""}
              onChange={(e) => crud.setFiltro("accion", e.target.value)}
            >
          ...
          <label>
            ...
            <input
              type="date"
              value={dbToInput(crud.filtros.fechaDesde || "")}
              onChange={(e) => crud.setFiltro("fechaDesde", inputToDb(e.target.value) || "")}
            />
          </label>
          <label>
            Hasta
            <input
              type="date"
              value={dbToInput(crud.filtros.fechaHasta || "")}
              onChange={(e) => crud.setFiltro("fechaHasta", inputToDb(e.target.value) || "")}
            />
          </label>
        </div>

        <DataTable
          columnas={auditoriaColumnas}
          items={crud.items}
          pk={cfg.pk}
          orden={crud.orden}
